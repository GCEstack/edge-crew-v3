...
import { supabase, getCurrentUser, fetchProfile, mapSupabaseToUser } from '@/lib/supabase';
      ...
      (set) => ({
        ...
        syncUserFromSession: async () => {
          try {
            ...
            const user = mapSupabaseToUser(authUser, profile);
            set({ user });
          } catch (err) {
            console.error('[syncUserFromSession] error:', err);
            set({ user: null });
          }
        },
        signOut: async () => {
