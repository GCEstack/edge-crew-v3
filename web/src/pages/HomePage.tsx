...
const CHINNY_AUDIO_URL =
  ((import.meta as any).env?.VITE_CHINNY_TAB_AUDIO_URL as string | undefined) || '/audio/chinny-theme.ogg'
...
export default function HomePage() {
  ...
  const gamesQueryKey = ['games', effectiveSport, apiMode, apiLeague, specialTab]
  ...
  const gameIds = Array.isArray(games) ? games.map((g: any) => g.id as string).filter(Boolean) : []
  const { status: jobStatus, progress: modelProgress, error: jobError } = useAnalysisJob({
    jobId: activeJobId,
  ...
  // Deep AI analysis — enqueue async job, then stream results via Realtime
  const handleAnalyzeAll = async () => {
    if (analyzing || activeJobId) return
    setAnalyzing(true)
    setAnalyzeError(null)
    setActiveJobId(null)

    try {
      const job = await analyzeGames(
        effectiveSport,
  ...
  useEffect(() => {
    if (jobStatus === 'completed' || jobStatus === 'failed') {
      setAnalyzing(false)
      if (jobError) setAnalyzeError(jobError)
      setActiveJobId(null)
    } else if (jobStatus === 'pending' || jobStatus === 'running') {
      setAnalyzing(true)
    }
  }, [jobStatus, jobError])
      ...
      {analyzing && (
        <div className="mb-4 p-3 bg-[#00E5FF]/5 border border-[#00E5FF]/20 rounded-lg text-sm text-[#00E5FF]">
          ...
          {modelProgress.length > 0 && (
            <span className="ml-1">({modelProgress.length} models finished)</span>
          )}
          ...
        </div>
      )}
