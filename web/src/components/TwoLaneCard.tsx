...
export function TwoLaneCard({ game, ourGrade, aiGrade, convergence }: TwoLaneCardProps) {
  ...
  const [analyzeBtnError, setAnalyzeBtnError] = useState<string | null>(null)
  const sportLower = (game.sport || '').toLowerCase()
  const { status: analyzeStatus, progress: analyzeProgress } = useAnalysisJob({
    jobId: analyzeJobId,
    sport: sportLower,
    gameIds: [game.id],
  ...
  useEffect(() => {
    if (analyzeStatus === 'completed') {
      ...
      setAnalyzeBtnError('Analysis failed')
      setAnalyzeJobId(null)
    }
  }, [analyzeStatus, queryClient])
          ...
          <div className="mt-2 flex flex-col items-center gap-1">
            <button
              onClick={async (e) => {
                ...
                setAnalyzeBtnError(null);
                try {
                  const job = await analyzeGame(sportLower, game.id);
                  setAnalyzeJobId(job.job_id);
                } catch (err) {
                  setAnalyzeBtnError((err as Error).message || 'Failed');
                }
              }}
              disabled={!!analyzeJobId}
              className="px-3 py-1 rounded-lg text-[10px] font-bold bg-[#00D4AA]/15 border border-[#00D4AA]/40 text-[#00D4AA] hover:bg-[#00D4AA]/25 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {analyzeJobId
                ? `Analyzing${analyzeProgress.length > 0 ? ` (${analyzeProgress.length})` : ''}...`
                : 'Analyze This Game'}
            </button>
            {analyzeBtnError && (
              <span className="text-[9px] text-rose-400">{analyzeBtnError}</span>
            )}
          </div>
        )}
        {pick && pick.side && (
