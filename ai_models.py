...
MOONSHOT_KIMI_MODEL = os.environ.get("MOONSHOT_KIMI_MODEL", "kimi-k2.6")
...
async def kimi_gatekeeper(game: dict, our_grade: dict, ai_grades: list, convergence: dict) -> dict:
    ...
    # Prefer Moonshot-hosted Kimi (fast + stable); fall back to Azure-hosted Kimi.
    has_moonshot = bool(MOONSHOT_API_KEY)
    has_azure = bool(GCE_KEY or AI_SERVICES_KEY)
    if not (has_moonshot or has_azure):
        return {"action": "?", "adjustment": 0, "reason": "Kimi unavailable — no MOONSHOT_API_KEY or AZURE_GCE_KEY"}
...
Your action — take a clear, decisive stance:
- CONFIRM (adj=0): the grade looks correct
- CHALLENGE (adj=-1 to -2): the grade is too high
- BOOST (adj=+1 to +2): the grade is too low and the edge is real
...
{{"action": "CONFIRM|CHALLENGE|BOOST", "adjustment": 0, "reason": "1-2 sentences taking a clear side — WHY", "verdict_tag": "SHORT_TAG"}}"""
    ...
    # Azure fallback routed through GCE_ENDPOINT (aoai_classic format).
    key = GCE_KEY
    base = f"{GCE_ENDPOINT.rstrip('/')}/deployments" if GCE_ENDPOINT else "https://gce-personal-resource.openai.azure.com/openai/deployments"
    ...
    if not key:
        return {
            "action": "?",
            "adjustment": 0,
            "reason": f"Moonshot gatekeeper failed ({kimi_err}) and no Azure key for fallback",
        }
