...
from data_fetch import enrich_game_for_grading, fetch_team_profile, SPORT_ESPN_MAP, SOCCER_LEAGUE_MAP as DF_SOCCER_LEAGUE_MAP
...
ODDS_API_ENABLED = bool(ODDS_API_KEY) and ODDS_API_KEY not in ("your-odds-api-key-here", "placeholder")
...
SPORT_KEYS = {
    ...
    "soccer": ["soccer_usa_mls", "soccer_epl", "soccer_spain_la_liga", "soccer_italy_serie_a", "soccer_germany_bundesliga", "soccer_france_ligue_one", "soccer_uefa_champs_league", "soccer_uefa_europa_league", "soccer_brazil_campeonato", "soccer_mexico_ligamx", "soccer_fifa_world_cup"],
...
_ESPN_MAP = {**SPORT_ESPN_MAP, "WNBA": ("basketball", "wnba")}
...
# Expanded roster - models across multiple hosts (gce / sweden / ai_peterwilson / gemini / perplexity / groq / moonshot / xai).
...
REAL_AI_MODELS = [
    ...
    {"display": "Grok 4.20 Reasoning", "deployment": "grok-4.20-0309-reasoning",              "host": "xai",         "persona": "newest xAI reasoning model, bleeding edge",  "token_param": "max_completion_tokens", "max_tokens": 8000,  "timeout": 240},
    {"display": "Grok 4.20",           "deployment": "grok-4.20-0309-non-reasoning",          "host": "xai",         "persona": "fast xAI generalist, contrarian takes",      "token_param": "max_completion_tokens", "max_tokens": 8000,  "timeout": 120},
    {"display": "Grok 4.3",            "deployment": "grok-4.3",                              "host": "xai",         "persona": "xAI flagship, balanced strategic view",      "token_param": "max_completion_tokens", "max_tokens": 8000,  "timeout": 180},
    {"display": "Grok Build 0.1",      "deployment": "grok-build-0.1",                        "host": "xai",         "persona": "xAI build model, cheap first-read scout",    "token_param": "max_completion_tokens", "max_tokens": 4000,  "timeout": 90},
    ...
    # Trimmed to a focused panel: one GPT-OSS, one Qwen, one Llama 4 Scout.
    {"display": "Groq GPT-OSS 120B",     "deployment": "openai/gpt-oss-120b",                 "host": "groq",        "persona": "OpenAI GPT-OSS heavy reasoning via Groq",    "token_param": "max_tokens",            "max_tokens": 2000,  "timeout": 120},
    {"display": "Groq Qwen3 32B",        "deployment": "qwen/qwen3-32b",                      "host": "groq",        "persona": "Qwen reasoning via Groq",                    "token_param": "max_tokens",            "max_tokens": 2000,  "timeout": 60},
    {"display": "Groq Llama-4 Scout",    "deployment": "meta-llama/llama-4-scout-17b-16e-instruct", "host": "groq",  "persona": "Llama 4 scout via Groq",                     "token_param": "max_tokens",            "max_tokens": 2000,  "timeout": 60},

    #    DeepSeek (direct API)
    {"display": "DeepSeek R1",         "deployment": "deepseek-reasoner",                   "host": "deepseek",    "persona": "data-driven heavy reasoner",                 "token_param": "max_tokens",            "max_tokens": 4000,  "timeout": 180},
    {"display": "DeepSeek V3",         "deployment": "deepseek-chat",                       "host": "deepseek",    "persona": "general-purpose DeepSeek chat",              "token_param": "max_tokens",            "max_tokens": 2500,  "timeout": 90},

    #    Moonshot (Kimi K2 via direct OpenAI-compatible API)
    {"display": "Moonshot Kimi K2.6",    "deployment": "kimi-k2.6",                         "host": "moonshot",    "persona": "long-context reasoner via Moonshot",         "token_param": "max_tokens",            "max_tokens": 6000,  "timeout": 180},
    {"display": "Moonshot Kimi K2.5",    "deployment": "kimi-k2.5",                         "host": "moonshot",    "persona": "prior-gen Kimi reasoning variant",           "token_param": "max_tokens",            "max_tokens": 6000,  "timeout": 180},
]


def _host_has_key(host: str) -> bool:
    cfg = AZURE_HOSTS.get(host, {})
...
def _active_real_models_for_sport(sport_upper: str, fast_mode: bool = False) -> list[dict]:
    ...
    active = [m for m in REAL_AI_MODELS if m.get("display") in keep and _host_has_key(m.get("host", ""))]
    # If the sport-specific panel is mostly unavailable (e.g., Azure disabled),
    # fall back to every provider that actually has a key so we still get a full mix.
    if len(active) < 6:
        active = [m for m in REAL_AI_MODELS if _host_has_key(m.get("host", ""))]
    return active[:10]
...
async def _grade_game_full(game: dict, sport_upper: str, odds_key: str = "") -> dict:
    ...
    if is_combat:
        ...
        try:
            ...
            logger.warning(
                f"Grade engine error for {game.get('homeTeam')} vs {game.get('awayTeam')}: {e}\n{traceback.format_exc()}"
            )
...
def _parse_espn_event(event: dict, sport_label: str) -> Optional[dict]:
    """Parse an ESPN scoreboard event into the same game dict shape as _parse_event."""
    ...
    if not home_team or not away_team:
        return None
    home_team = TEAM_NAME_OVERRIDES.get(home_team, home_team)
    away_team = TEAM_NAME_OVERRIDES.get(away_team, away_team)

    commence = competition.get("date") or event.get("date", "")
    status_type = (competition.get("status") or event.get("status") or {}).get("type", {})
    state = (status_type.get("state") or "").lower()
    status_id = str(status_type.get("id") or "")
    status = "scheduled"
    if state == "in":
        status = "live"
    elif state == "post" or status_id == "3":
        status = "completed"

    spread = total = ml_home = ml_away = None
    spread_price_home = spread_price_away = -110
    over_price = under_price = None
    draw_price = None
    bookmaker_used = None
    for odd in (competition.get("odds") or []):
        provider = (odd.get("provider") or {}).get("name", "")
        if spread is None and odd.get("spread") is not None:
            spread = float(odd["spread"])
        if total is None and odd.get("overUnder") is not None:
            total = float(odd["overUnder"])
        ml_block = odd.get("moneyline") or {}
        if ml_home is None:
            home_ml = ml_block.get("home") or {}
            raw = (home_ml.get("close") or {}).get("odds")
            if raw:
                ml_home = int(str(raw).replace("+", ""))
        if ml_away is None:
            away_ml = ml_block.get("away") or {}
            raw = (away_ml.get("close") or {}).get("odds")
            if raw:
                ml_away = int(str(raw).replace("+", ""))
        ps_block = odd.get("pointSpread") or {}
        if spread_price_home == -110:
            home_ps = ps_block.get("home") or {}
            raw = (home_ps.get("close") or {}).get("odds")
            if raw:
                spread_price_home = int(str(raw).replace("+", ""))
        if spread_price_away == -110:
            away_ps = ps_block.get("away") or {}
            raw = (away_ps.get("close") or {}).get("odds")
            if raw:
                spread_price_away = int(str(raw).replace("+", ""))
        total_block = odd.get("total") or {}
        if over_price is None:
            over = total_block.get("over") or {}
            raw = (over.get("close") or {}).get("odds")
            if raw:
                over_price = int(str(raw).replace("+", ""))
        if under_price is None:
            under = total_block.get("under") or {}
            raw = (under.get("close") or {}).get("odds")
            if raw:
                under_price = int(str(raw).replace("+", ""))
        if draw_price is None and ml_block.get("draw"):
            raw = (ml_block["draw"].get("close") or {}).get("odds")
            if raw:
                draw_price = int(str(raw).replace("+", ""))
        if not bookmaker_used and provider:
            bookmaker_used = provider.lower().replace(" ", "")
        if ml_home is not None and total is not None:
            break

    away_spread = -spread if spread is not None else 0
    odds = {
        "spread": away_spread,
        "total": total or 0,
        "mlHome": ml_home or 0,
        "mlAway": ml_away or 0,
        "spreadPriceHome": spread_price_home,
        "spreadPriceAway": spread_price_away,
        "overPrice": over_price,
        "underPrice": under_price,
        "bttsYes": None,
        "bttsNo": None,
    ...
    stable_id = hashlib.md5(
        f"{home_team}|{away_team}|{commence}".encode("utf-8")
    ).hexdigest()[:16]
    return {
        "id": stable_id,
        "oddsApiId": str(event.get("id", "")),
        "sport": sport_label,
        "homeTeam": home_team,
        "awayTeam": away_team,
        "scheduledAt": commence,
...
async def _fetch_games_from_espn(sport: str, league: str = "") -> list:
    """Fetch the next 24-48h slate from ESPN scoreboard (today + tomorrow)."""
    sport_lower = sport.lower()
    sport_upper = sport_lower.upper()
    endpoints: list[tuple[str, str]] = []

    if sport_lower == "soccer":
        if league and league in SOCCER_LEAGUE_MAP:
            odds_keys = SOCCER_LEAGUE_MAP[league]
        else:
            odds_keys = SPORT_KEYS.get("soccer", [])
        for key in odds_keys:
            mapped = DF_SOCCER_LEAGUE_MAP.get(key)
            if mapped:
                endpoints.append(mapped)
    else:
        mapped = _ESPN_MAP.get(sport_upper)
        if mapped:
            endpoints = [mapped]

    if not endpoints:
        logger.warning(f"[ESPN] No ESPN mapping for sport={sport_lower}")
        return []
    ...
    all_games: list[dict] = []
    async with httpx.AsyncClient(timeout=15) as client:
        for espn_sport, espn_league in endpoints:
            for date_str in dates:
                url = f"https://site.api.espn.com/apis/site/v2/sports/{espn_sport}/{espn_league}/scoreboard?dates={date_str}"
                try:
                    resp = await client.get(url)
                    if resp.status_code != 200:
                        logger.warning(f"[ESPN] {espn_sport}/{espn_league}/{date_str}: HTTP {resp.status_code}")
                        continue
                    events = resp.json().get("events", [])
                    logger.info(f"[ESPN] {espn_sport}/{espn_league}/{date_str}: {len(events)} events")
                    for event in events:
                        game = _parse_espn_event(event, sport_upper)
                        if not game:
                            continue
                        ...
                        try:
                            ...
                            if ct:
                                ...
                                if hours_ahead < -6 or hours_ahead > 24:
                                    continue
                        except Exception:
                            pass
                        game["_sport_key"] = f"{espn_sport}/{espn_league}"
                        all_games.append(game)
                except Exception as e:
                    logger.warning(f"[ESPN] {espn_sport}/{espn_league}/{date_str}: {e}")
    ...
    if all_games:
        ...
        if len(deduped) != len(all_games):
            logger.info(f"[DEDUPE] {sport_lower}: {len(all_games)} -> {len(deduped)} games after dedupe")
        all_games = deduped
    return all_games


async def _fetch_and_grade(sport: str, mode: str = "games", league: str = "") -> list:
    """Fetch live games, then grade each one. Uses Odds API if configured, otherwise ESPN."""
    sport_lower = sport.lower()
    sport_upper = sport_lower.upper()
    all_games: list[dict] = []
    ...
    if sport_upper == "GOLF":
        if ODDS_API_ENABLED:
            keys = SPORT_KEYS.get("golf", [])
            return await _fetch_golf_outrights(keys)
        logger.warning("[FETCH+GRADE] Golf requires Odds API; skipping")
        return []

    if ODDS_API_ENABLED:
        # Soccer league filtering
        if sport_lower == "soccer" and league and league in SOCCER_LEAGUE_MAP:
        ...
        logger.info(f"[FETCH+GRADE] Odds API disabled; fetching {sport_lower} slate from ESPN")
        all_games = await _fetch_games_from_espn(sport_lower, league=league)

    # Dedupe by stable game id
    if all_games:
        seen_ids = set()
        deduped = []
        for g in all_games:
    ...
    # Soccer BTTS only available through Odds API
    if ODDS_API_ENABLED and sport_upper == "SOCCER" and all_games:
    ...
    try:
        ...
        logger.warning(
            f"[FETCH+GRADE] HARD TIMEOUT (>{grade_timeout}s) for {sport_lower} - returning whatever finished"
        )
...
async def _analyze_games_impl(request: AnalyzeRequest):
    ...
    _any_ai_key = any(_host_has_key(h) for h in AZURE_HOSTS)
    logger.info(
        f"[ANALYZE] Deep analysis for {sport_lower} league={league_key or 'all'}: "
        f"{len(games)} games (real AI={'on' if _any_ai_key else 'OFF - fallback'}, fast={fast_mode})"
    )
